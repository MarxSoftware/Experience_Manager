...
//		}
	...
	function custom_attributes($attrs, $module) {
		...
		if ($this->isActivated($module)) {
			...
			/*
			  if ($module->settings->tma_default === "no" && !is_user_logged_in()) {
			  $attrs["class"] = $attrs["class"] . ' tma-hide';
			  }
			 */
			if (!\FLBuilderModel::is_builder_active() && !tma_exm_admin_bar_visible()) {
